...
import pytest
from crategen.models.tes_models import TESExecutorLog, TESExecutor, TESData, TESInput, TESOutput, TESTaskLog, TESOutputFileLog
...
class TestTESExecutorLog:
    ...
    def test_validate_datetime_valid(self):
      ...
      # Create a TESExecutorLog object (we're just interested in the validator)
        ...
        assert bool(log_entry.start_time) == True
        assert bool(log_entry.end_time) == True
    ...
    def test_validate_datetime_invalid(self):
      ...
      for invalid_datetime in invalid_datetime_strings:
          ...
          assert "The 'start_time' property must be in the rfc3339 format" in str(exc_info.value)
          ...
          assert "The 'end_time' property must be in the rfc3339 format" in str(exc_info.value)
...
class TestTESExecutor:
  ...
  def test_validate_stdin_stdout_valid(self):
    ...
    for path in valid_paths:
      executor = TESExecutor(image="image", command=["commands"], stdin=path, stdout=path)

      assert bool(executor) == True
...
class TestTESTInput:
  ...
  def test_validate_path_valid(self):
    ...
    for valid_path in valid_paths:
      ...
      assert bool(input_data.path) == True
  ...
  def test_validate_no_content_or_url(self):
    ...
    assert "Either the 'url' or 'content' properties must be set" in str(exc_info.value)
  ...
  def test_validate_content_or_url(self):
     ...
     assert bool(tes_input.url) == True
     assert bool(tes_input.content) == False

     tes_input = TESInput(path="/", content="content")

     assert bool(tes_input.url) == False
     assert bool(tes_input.content) == True
...
class TestTESTaskLog:
  ...
  def test_validate_datetime_valid(self):
    ...
    for time in valid_datetime_strings:
      tes_task_log = TESTaskLog(outputs=[self.tes_output_file_log], logs=[self.tes_executor_log], start_time=time, end_time=time)

      assert bool(tes_task_log.start_time) == True
  ...
  def test_validate_datetime_invalid(self):
    ...
    for time in invalid_datetime_strings:
      with pytest.raises(ValueError) as exc_info:
        TESTaskLog(outputs=[self.tes_output_file_log], logs=[self.tes_executor_log], start_time="2020-10-02T16:00:00.000Z", end_time=time)

      assert "The 'end_time' property must be in the rfc3339 format" in str(exc_info.value)
      
      with pytest.raises(ValueError) as exc_info:
        TESTaskLog(outputs=[self.tes_output_file_log], logs=[self.tes_executor_log], start_time=time, end_time="2020-10-02T16:00:00.000Z")
...
class TestTESData:
  ...
  def test_validate_datetime_valid(self):
    ...
    for valid_datetime in valid_datetime_strings:
        data = TESData(
            id="id",
            creation_time=valid_datetime, 
            executors=[self.executor]
        )
        assert bool(data.creation_time) == True
  ...
  def test_validate_datettime_invalid(self):
    ...
    for invalid_datetime in invalid_datetime_strings:
      with pytest.raises(ValueError) as exc_info:
        TESData(
                id="id",
                creation_time=invalid_datetime, 
                executors=[self.executor]
            )
        
      assert "The 'creation_time' property must be in the rfc3339 format" in str(exc_info.value)
